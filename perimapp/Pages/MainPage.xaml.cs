    ...
    public partial class MainPage : ContentPage
    {
        ...
        private async void OnProductSelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (e.CurrentSelection != null && e.CurrentSelection.Any())
            {
                ...
                if (selectedProduct != null)
                {
                    ...
                    await Shell.Current.GoToAsync($"{nameof(DetailsPage)}?productId={selectedProduct.Id}");
                }
            }
        }
        ...
        private async Task LoadProductsAsync()
        {
            try
            {
                ...
                List<ProductInfos> loadedProducts = await _productService.GetUserProductsAsync(AppData.CurrentUserId);
        ...
        private async Task<List<ProductInfos>> LoadProductsFromJsonAsync()
        {
            try
            {
                ...
                var products = System.Text.Json.JsonSerializer.Deserialize<List<ProductInfos>>(jsonContent);
                return products;
            }
            catch (Exception ex)
            {
